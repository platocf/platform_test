package com.cn.platform.common.model;

import java.util.Date;
/***
 * 用户基本属性--->可以继承
 * @author era
 *
 */
public class user {
	  private int U_ID;
	  private String U_LoginName;
	  private String U_Password;
	  private String U_CName;
	  private String U_EName;
	  private String U_Email;
	  private int U_RoleId;
	  private int U_Status;
	  private String U_Remark;
	  private Date U_BirthDay;
	  private int U_Sex;
	  private String U_IDCard;
	  private String U_MobileNo;
	  private String U_PhotoUrl;
	  private Date U_DateTime;
	  private String U_LastIP;
	  private Date U_LastDateTime;
	  private String U_Address;
	  private String U_GetPassword;
	public String getU_GetPassword() {
		return U_GetPassword;
	}
	public void setU_GetPassword(String u_GetPassword) {
		U_GetPassword = u_GetPassword;
	}
	public int getU_ID() {
		return U_ID;
	}
	public void setU_ID(int u_ID) {
		U_ID = u_ID;
	}
	public String getU_LoginName() {
		return U_LoginName;
	}
	public void setU_LoginName(String u_LoginName) {
		U_LoginName = u_LoginName;
	}
	public String getU_Password() {
		return U_Password;
	}
	public void setU_Password(String u_Password) {
		U_Password = u_Password;
	}
	public String getU_CName() {
		return U_CName;
	}
	public void setU_CName(String u_CName) {
		U_CName = u_CName;
	}
	public String getU_EName() {
		return U_EName;
	}
	public void setU_EName(String u_EName) {
		U_EName = u_EName;
	}
	public String getU_Email() {
		return U_Email;
	}
	public void setU_Email(String u_Email) {
		U_Email = u_Email;
	}
	public int getU_RoleId() {
		return U_RoleId;
	}
	public void setU_RoleId(int u_RoleId) {
		U_RoleId = u_RoleId;
	}
	public int getU_Status() {
		return U_Status;
	}
	public void setU_Status(int u_Status) {
		U_Status = u_Status;
	}
	public String getU_Remark() {
		return U_Remark;
	}
	public void setU_Remark(String u_Remark) {
		U_Remark = u_Remark;
	}
	public Date getU_BirthDay() {
		return U_BirthDay;
	}
	public void setU_BirthDay(Date u_BirthDay) {
		U_BirthDay = u_BirthDay;
	}
	public int getU_Sex() {
		return U_Sex;
	}
	public void setU_Sex(int u_Sex) {
		U_Sex = u_Sex;
	}
	public String getU_IDCard() {
		return U_IDCard;
	}
	public void setU_IDCard(String u_IDCard) {
		U_IDCard = u_IDCard;
	}
	public String getU_MobileNo() {
		return U_MobileNo;
	}
	public void setU_MobileNo(String u_MobileNo) {
		U_MobileNo = u_MobileNo;
	}
	public String getU_PhotoUrl() {
		return U_PhotoUrl;
	}
	public void setU_PhotoUrl(String u_PhotoUrl) {
		U_PhotoUrl = u_PhotoUrl;
	}
	public Date getU_DateTime() {
		return U_DateTime;
	}
	public void setU_DateTime(Date u_DateTime) {
		U_DateTime = u_DateTime;
	}
	public String getU_LastIP() {
		return U_LastIP;
	}
	public void setU_LastIP(String u_LastIP) {
		U_LastIP = u_LastIP;
	}
	public Date getU_LastDateTime() {
		return U_LastDateTime;
	}
	public void setU_LastDateTime(Date u_LastDateTime) {
		U_LastDateTime = u_LastDateTime;
	}
	public String getU_Address() {
		return U_Address;
	}
	public void setU_Address(String u_Address) {
		U_Address = u_Address;
	}
	  
}
